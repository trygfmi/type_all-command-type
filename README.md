...
<p>このリポジトリは、typeコマンドを実行して引数に指定したコマンドに設定されている実行方法を表示します</p>
</div>
...
<div class="wp-block-group">
<p>typeコマンドを実行して引数に指定したコマンドに設定されている実行方法を表示してみてください</p>
...
<div class="wp-block-group">
<pre class="wp-block-own-copy-code-line-block wp-block-code has-24292-eff-color has-cyan-bluish-gray-background-color has-text-color has-background has-1-125-rem-font-size"><code>type -a echo</code></pre>
...
<div class="wp-block-group">
<p>shell builtinはシェルに標準で実装されているのでパスを検索する必要がなく高速に動作します<br>以下のコマンドを実行するとエイリアスが設定されビルトインよりも優先して実行されます</p>



<pre class="wp-block-own-copy-code-line-block wp-block-code has-24292-eff-color has-cyan-bluish-gray-background-color has-text-color has-background has-1-125-rem-font-size"><code>alias echo="echo 'hello type command world'"
type -a echo
echo</code></pre>
</div>
